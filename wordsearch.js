...
{ "word": "LAKE", "direction": "SE", "start": 3 },
...
{ "word": "MOBILE", "direction": "W", "start": 99 },
...
var words2 = [{ "word": "BUFFALO2222", "direction": "N", "start": 254 },
{ "word": "LAKE", "direction": "SE", "start": 3 },
{ "word": "PRECIPITATE", "direction": "NE", "start": 323 },
{ "word": "CALDRON", "direction": "S", "start": 39 },
{ "word": "MISCIBLE", "direction": "NW", "start": 268 },
...
function generateUniqueRandomNumbers(count, max) {
	const uniqueNumbers = new Set(); // Use a Set to handle unique values

	while (uniqueNumbers.size < count) {
		const random = Math.floor(Math.random() * max); // Generate a random number from 0 to max-1
		uniqueNumbers.add(random);
	}

	return Array.from(uniqueNumbers); // Convert Set to Array
}

const randomNumbers = generateUniqueRandomNumbers(20, 1000); // Adjusted max to 1000 to get numbers from 0 to 999


let board = Array.from({ length: 20 }, () => Array(20).fill(''));
function isValidPlacement(grid, word, startX, startY, direction) {
	const length = word.length;
	let x = startX;
	let y = startY;

	for (let i = 0; i < length; i++) {
		if (x < 0 || x >= 20 || y < 0 || y >= 20 || grid[y][x] !== '') {
			return false;
		}
...
function findPlacementForWord(board, word) {
	const directions = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'];
	const maxAttempts = 1000; // Giới hạn số lần thử để tránh lặp vô hạn
	let attempts = 0;

	while (attempts < maxAttempts) {
		const direction = directions[Math.floor(Math.random() * directions.length)];
		const start = Math.floor(Math.random() * 400);
		if (isValidPlacement(board, word, start % 20, Math.floor(start / 20), direction)) {
			return { word: word, start: start, direction: direction };
		}
		attempts++;
	}

	return null; // Trả về null nếu không tìm được vị trí hợp lý sau nhiều lần thử
}
for (var i = 0; i < 20; i++) {
	words[i].word = Englishvoca[randomNumbers[i]]
	const result = findPlacementForWord(board, Englishvoca[randomNumbers[i]]);
	words[i].direction = result.direction
	words[i].start = result.start
	if (result) {
		console.log("Từ được đặt:", words[i].start);
	} else {
		console.log("Không thể đặt từ");
	}
}
for (var i = 0; i < 20; i++) {
	words2[i].word = KoreanVoca[randomNumbers[i]]
}
...
$(document).ready(function () {
	...
	var size = 320; //($(".left").css("width").slice(0, 3) - 20) / 2 ;
	...
	for (var i = 0; i < words.length; i++) {
		...
		$(".words").append("<button onclick = speakText('" + words[i].word + "') class='" + (i) + "'>" +
			words[i].word.trim().toUpperCase() + "</button> </br>");
	}
	for (var i = 0; i < words2.length; i++) {
		$(".words2").append("<button onclick = speakText2('" + words2[i].word + "') class='" + (i) + "'>" +
			words2[i].word + "</button> </br>");
	}
...
function displayWord(w) {
	for (var j = 0; j < w.word.length; j++) {
		if (w.direction == "N") {
			$(".letters").find("." + w.end).text(w.word[j].trim().toUpperCase());
...
function scratchWord() {
	for (var i = 0; i < words.length; i++) {
		if ((click.startPos === words[i].start && click.endPos === words[i].end) ||
			...
			// little hack here
			speakText($(".words").find("." + i).text());

			(function (index) {
				setTimeout(function () {
					speakText2($(".words2").find("." + index).text());  // Speak after delay using the captured index
				}, 1000);
			})(i);
			$(".words").find("." + i).addClass("strike");
			$(".words2").find("." + i).addClass("strike");
		}
	}
	// check if the game is over
